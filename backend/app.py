...
def clasificacion(grupo: str | None = None) -> list[dict[str, Any]]:
    ...
    return [r for r in rows if str(r.get("group", "")).upper() == grupo.upper()] if grupo else rows

@app.get("/api/goleadores")
def goleadores(limit: int = 50) -> list[dict[str, Any]]:
    rows = sorted(DATA.get("top_scorers", []) or [], key=lambda r: int(r.get("goals") or 0), reverse=True)
